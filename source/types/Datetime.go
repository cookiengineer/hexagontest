...
func isISO8601(value string) bool {
	...
	if strings.Contains(value, "T") && strings.HasSuffix(value, "Z") {
		...
		date := strings.Split(value, "T")[0]
		time := strings.Split(value, "T")[1]
		...
		check_date := strings.Split(date, "-")
		check_time := strings.Split(time, ":")
...
func toChunks(value string) []string {
	...
	for v := 0; v < len(values); v++ {
		...
		value := strings.TrimSpace(values[v])
...
func ParseDatetime(value string) *Datetime {
	...
	var result *Datetime = nil
	var datetime Datetime

	if value != "" && datetime.Parse(value) == true {
		result = &datetime
	}
...
func (datetime *Datetime) Offset(offset string) {
	...
	if err_hours == nil && err_minutes == nil {
		...
		if operator == "+" {
			...
			day := datetime.Day
			hour := int(datetime.Hour - uint(offset_hours))
			minute := int(datetime.Minute - uint(offset_minutes))
			...
			month := datetime.Month
			day := datetime.Day
			hour := datetime.Hour + uint(offset_hours)
			minute := datetime.Minute + uint(offset_minutes)
			...
			monthdays := uint(MONTHDAYS[month-1])
...
func (datetime *Datetime) Parse(value string) bool {
	...
	if isISO8601(strings.TrimSpace(value)) {
		...
		if strings.Contains(chunks[0], "T") {
			...
			time_suffix := strings.Split(chunks[0], "T")[1]
...
func (datetime *Datetime) ToDays() uint {
	...
	if isLeapYear(year) && month == 2 {
		result = uint(MONTHDAYS[month-1]) + 1
	} else {
		result = uint(MONTHDAYS[month-1])
	}
...
func (datetime *Datetime) ToDatetimeDifference(other Datetime) Datetime {
	...
	if datetime.IsBefore(other) {
		...
		months := uint(0)
		days := uint(0)
		hours := uint((24 - datetime.Hour) + other.Hour)
		minutes := uint((60 - datetime.Minute) + other.Minute)
		seconds := uint((60 - datetime.Second) + other.Second)
...
func (datetime *Datetime) ToWeekday() string {
	...
	month := datetime.Month
	day := datetime.Day
	offsets := []uint{0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4}

	if month < 3 {
		year -= 1
	}

	index := uint((year + year/4 - year/100 + year/400 + offsets[month-1] + day) % 7)
